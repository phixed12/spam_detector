...
def process(df: pd.DataFrame, cfg: dict, verbose: bool = False,
            on_progress=None) -> pd.DataFrame:
    """
    on_progress(fraction: float, message: str) — optional callback called at
    key steps with a value in [0, 1] so callers can drive a progress bar.
    """
    ...
    # --- Run all rules, ticking progress before each ---
    _RULES = [
        ("High-risk industry",        lambda: rule_high_risk_industry(df, col_map, cfg)),
        ("Generic name keywords",     lambda: rule_generic_name(df, col_map, cfg)),
        ("PO Box",                    lambda: rule_po_box(df, col_map, cfg)),
        ("Virtual mailbox",           lambda: rule_virtual_mailbox(df, col_map, cfg)),
        ("Shared address",            lambda: rule_shared_address(df, col_map, cfg, shared_address_counts)),
        ("State / ZIP mismatch",      lambda: rule_state_zip_mismatch(df, col_map, cfg)),
        ("Phone validation",          lambda: rule_invalid_phone(df, col_map, cfg)),
        ("Toll-free phone",           lambda: rule_tollfree_phone(df, col_map, cfg)),
        ("Shared phone",              lambda: rule_shared_phone(df, col_map, cfg, shared_phone_counts)),
        ("Area code mismatch",        lambda: rule_area_code_mismatch(df, col_map, cfg)),
        ("Website presence",          lambda: rule_no_website(df, col_map, cfg)),
        ("Website quality",           lambda: rule_website_quality(df, col_map, cfg, shared_domain_counts)),
        ("Free email provider",       lambda: rule_free_email(df, col_map, cfg)),
        ("Auto-generated email",      lambda: rule_autogenerated_email(df, col_map, cfg)),
        ("Shared email",              lambda: rule_shared_email(df, col_map, cfg, shared_email_counts)),
        ("Email domain mismatch",     lambda: rule_email_domain_mismatch(df, col_map, cfg)),
        ("Exact duplicates",          lambda: rule_exact_duplicate(df, col_map, cfg)),
        ("Near-duplicate names",      lambda: rule_near_duplicate_name(df, col_map, cfg, norm_names)),
        ("Same owner",                lambda: rule_same_owner(df, col_map, cfg)),
        ("Batch submission",          lambda: rule_batch_submission(df, col_map, cfg)),
        ("Photo gallery",             lambda: rule_no_photos(df, col_map, cfg)),
        ("Social media presence",     lambda: rule_no_social(df, col_map, cfg)),
        ("Year established",          lambda: rule_year_established(df, col_map, cfg)),
        ("Business hours",            lambda: rule_no_hours(df, col_map, cfg)),
        ("Description quality",       lambda: rule_description_quality(df, col_map, cfg)),
        ("Address hidden",            lambda: rule_address_hidden(df, col_map, cfg)),
        ("Service area",              lambda: rule_service_area_no_address(df, col_map, cfg)),
        ("Keyword stuffing",          lambda: rule_keyword_field_stuffed(df, col_map, cfg)),
        ("Landing page domain",       lambda: rule_landing_page_domain_mismatch(df, col_map, cfg)),
        ("Residential address (RDI)", lambda: rule_residential_address(df, col_map, cfg, rdi_cache)),
    ]
    _N = len(_RULES)
    all_results: list[RuleResult] = []
    for _i, (_label, _fn) in enumerate(_RULES):
        _tick(0.22 + 0.65 * _i / _N, f"Checking: {_label}…")
        all_results += _fn()

    _tick(0.90, "Aggregating scores…")
    ...
    _tick(0.96, "Building output columns…")
